...
{
    [MenuItem("Tools/Infinity Engine/Built-in styles and icons")]
    private static void ShowWindow()
    {
        GetWindow<BuiltInResourcesWindow>().Show();
    }
    ...
    void OnGUI()
    {
        if (Math.Abs(position.width - _oldPosition.width) > double.Epsilon && Event.current.type == EventType.Layout)
        {
            Drawings = null;
